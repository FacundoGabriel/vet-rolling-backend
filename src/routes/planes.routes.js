...
const { aniadirPlan, cancelarPlan, cancelarPlanComoVeterinario, obtenerPlanesVeterinario, crearPlan, obtenerPlanes, obtenerUnPlan, editarPlan, eliminarPlan, agregarImagenPlan } = require("../controllers/planes.controllers");
...
router.post("/crearPlan",[
    ...
    .isArray({ min: 1 }),
], validarCampos,  auth("admin"), crearPlan)
router.put("/agregarImagen/:id",
     multerMiddlewares.single("imagen"), auth("admin"), agregarImagenPlan)
...
router.put("/editarPlan/:idPlan", [
    ...
    .isArray({ min: 1 }),
    check("idPlan", "ID incorrecto. Formato no corresponde a mongoose").isMongoId()
], validarCampos,auth("admin"), editarPlan)
router.delete("/eliminarPlan/:idPlan",  [
    check("idPlan", "ID incorrecto. Formato no corresponde a mongoose").isMongoId()
], validarCampos, auth("admin"), eliminarPlan)
